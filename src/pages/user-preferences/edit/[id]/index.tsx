...
import { useRoqClient, useUserPreferenceFindFirst } from 'lib/roq';
...
function UserPreferenceEditPage() {
  ...
  const { data, error, isLoading, mutate } = useUserPreferenceFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: UserPreferenceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.user_preference.update({
        data: values as RoqTypes.user_preference,
        where: {
          id,
        },
      });
