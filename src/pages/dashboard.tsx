...
import { useRoqPlatformUserProfiles, useLocationCount, useUserPreferenceCount, useWeatherDataCount } from 'lib/roq';

import { LocationListPage as ShowCaseList } from 'pages/locations';
...
function DashboardPage() {
  ...
  const userPreferenceStat = useUserPreferenceCount();

  const weatherDataStat = useWeatherDataCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: userPreferenceStat?.data,
      isLoading: userPreferenceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Weather Data',
      result: weatherDataStat?.data,
      isLoading: weatherDataStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`locations`} passHref legacyBehavior>
                  <Button
